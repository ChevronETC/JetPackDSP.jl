...
function JopEnvelope(spc::JetSpace{T,N}, power::Real=1.0) where {T,N}
    JopNl(dom = spc, rng = spc, f! = JopEnvelope_f!, df! = JopEnvelope_df!, df′! = JopEnvelope_df′!,
        upstate! = JopEnvelope_upstate!, s = (power=T(power/2), eu=zeros(spc), hu=zeros(spc)))
end
export JopEnvelope

function JopEnvelope_hu_and_eu!(m::AbstractArray, hu, eu)
    hu .= imag.(hilbert(m))
    eu .= m.^2 .+ hu.^2
end

JopEnvelope_upstate!(m::AbstractArray, s::NamedTuple) = JopEnvelope_hu_and_eu!(m, s.hu, s.eu)

function JopEnvelope_f!(d::AbstractArray, m::AbstractArray; power, hu, eu)
    JopEnvelope_hu_and_eu!(m, hu, eu)
    d .= eu.^power
end
